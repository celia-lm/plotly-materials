...
server = app.server
app.title = 'Plotly Annotations Tutorial'
...
credits_text = '''
    This app was created by [Celia López](https://www.linkedin.com/in/celia-lopez-monreal/?locale=en_US), member of Plotly's Customer Success Team.
    Check the [Tutorial Post on the Community Forum](https://community.plotly.com/t/tutorial-on-legend-and-annotations-positioning/65160).
...
app.layout = html.Div([
    dbc.Row([
        dbc.Accordion(dbc.AccordionItem(
            ...
            children=[
                dcc.Markdown(intro_text, style={'margin':'15px'}),
                dcc.Markdown(credits_text, style={'margin':'15px', 'font-size':'small'})
                ]
        )),
        dbc.Accordion(dbc.AccordionItem(
    ...
    if current_state.get('annotation') == 'legend' :
        ...
        if trigger  == 'annotation' :
            for i in default_values.keys():
                print(fig['layout']['annotations'][0][i])
                if fig['layout']['annotations'][0][i] is None :
                    current_state[i] = default_values.get(i)
                else :
                    current_state[i] = fig['layout']['annotations'][0][i] 
        else  :
